...
namespace glutils {
	...
	glm::dvec3 crossProduct(const glm::dvec3& a, const glm::dvec3& b) {
	...
	void drawPolygon(const std::vector<glm::dvec3>& points, const glm::vec4& color, const glm::mat4& mat, std::vector<Vertex>& vertices) {
		glm::dvec3 p1(mat * glm::vec4(points.back(), 1));
		glm::dvec3 p2(mat * glm::vec4(points[0], 1));

		for (int i = 0; i < points.size() - 2; ++i) {
			glm::dvec3 p3(mat * glm::vec4(points[i + 1], 1));

			// calculate the normal vector
			glm::dvec3 normal = glm::cross(p2 - p1, p3 - p2);
